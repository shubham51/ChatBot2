...
def get_ques_ans(file_name):
    ...
    x_text = inbounds_and_outbounds.text_x.apply(lambda txt: sn_re.sub(sn_replace, txt))
    y_text = inbounds_and_outbounds.text_y.apply(lambda txt: sn_re.sub(sn_replace, txt))
    p.set_options(p.OPT.URL, p.OPT.EMOJI, p.OPT.HASHTAG, p.OPT.SMILEY)
    x_text = x_text.tolist()
    y_text = y_text.tolist()
    for i in range(len(x_text)):
        x_text[i] = p.clean(x_text[i]).lower()
    for i in range(len(y_text)):
        y_text[i] = p.clean(y_text[i]).lower()
...
def prepare_raw_data():
    ...
    questions, answers = get_ques_ans('/home/shubham/Desktop/twcs.csv')
